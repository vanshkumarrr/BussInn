...
const Login = () => {
  ...
  const routeUserByRole = async (clerkUserId, loggedInEmail) => {
    console.log("Looking up Supabase profile:", clerkUserId);

    if (loggedInEmail.toLowerCase() === ADMIN_EMAIL) {
      localStorage.setItem("bussinn_role", "admin");
      navigate({
        to: "/admin/overview",
        replace: true
      });
  ...
  const handleLogin = async (e) => {
    ...
    if (
      ...
      try {
        localStorage.setItem("bussinn_role", "admin");
        localStorage.setItem("bussinn_user_email", ADMIN_EMAIL);
        navigate({
          to: "/admin/overview",
          replace: true
        });
        ...
        setErrorMessage("Unable to sign in as admin.");
      } finally {
        setIsLoading(false);
      }
      return;
    ...
    try {
      ...
      if (signIn.status !== "complete") {
        setErrorMessage(`Unable to complete sign in. Status: ${signIn.status}`);
        return;
      }
            ...
            <form className="login-form" onSubmit={handleLogin}>
              ...
              <div className="login-actions">
                <button type="submit" className="btn-primary" disabled={isLoading}>
                  {isLoading ? t.loadingText : t.signInBtn}
                </button>
